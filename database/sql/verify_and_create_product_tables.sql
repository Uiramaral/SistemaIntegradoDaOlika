-- ============================================
-- VERIFICAR E CRIAR TABELAS DE PRODUTOS
-- (Alergénicos já existem - não criar)
-- ============================================

-- 1. Verificar se tabela allergens existe (deve existir)
SELECT 
    CASE 
        WHEN EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'allergens')
        THEN 'Tabela allergens encontrada - OK!'
        ELSE 'ATENÇÃO: Tabela allergens NÃO encontrada! Execute o arquivo allergens.sql primeiro.'
    END AS status_allergens;
...
SELECT 'Configuração de produtos atualizada!' AS status;
