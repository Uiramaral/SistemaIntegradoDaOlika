...
@section('content')
    ...
    <div class="bg-card rounded-xl border border-border animate-fade-in w-full overflow-x-hidden" id="recipes-page"
        ...
        {{-- Recipes Grid --}}
        <div id="recipes-grid" class="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                ...
                <div class="recipe-card border border-border rounded-xl p-4 hover:shadow-md transition-all cursor-pointer group flex flex-col justify-between bg-white w-full min-w-0"
                    ...
                    <div class="flex items-start justify-between mb-4 gap-3">
                        <div class="flex items-center gap-3 min-w-0 flex-1">
                            <div
                                class="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center overflow-hidden shrink-0 border border-primary/20">
                                <span class="text-primary font-bold">{{ $initials }}</span>
                            </div>
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            <button @click.stop="$dispatch('open-recipe-modal', { id: {{ $recipe->id }} })"
                    ...
                    <div class="pt-3 border-t border-border mt-auto">
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <div class="text-right min-w-0">
                                <p class="text-[10px] text-muted-foreground uppercase tracking-widest font-semibold truncate">
                                    {{ $finalPrice ? 'VENDA' : 'REND.' }}
                                </p>
                                <p
                                    class="text-sm font-medium mt-0.5 {{ $finalPrice ? 'text-green-600' : 'text-foreground' }} truncate">
                                    {{ $finalPrice ? 'R$ ' . number_format($finalPrice, 2, ',', '.') : number_format($totalWeight, 0, ',', '.') . 'g' }}
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <div class="px-4 sm:px-6 py-3 sm:py-4 border-t border-border bg-muted/20">
            <div class="flex flex-col sm:flex-row items-center justify-center lg:justify-center gap-3 sm:gap-4">
                <p class="text-xs text-muted-foreground font-medium order-2 sm:order-1 lg:order-1 text-center sm:text-left">
                    Mostrando <span class="font-bold text-foreground">{{ $recipes->firstItem() ?? $recipes->count() }}</span> de
                    <span class="font-bold text-foreground">{{ $recipes->total() }}</span> receitas
                </p>
    ...
    <div id="recipe-form-modal" x-data="recipeModal()" x-show="isOpen" @open-recipe-modal.window="openModal($event.detail)"
        @keydown.escape.window="closeModal()"
        class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" x-cloak
        style="display: none;">
        <div class="bg-card rounded-2xl border border-border shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg sm:text-xl font-bold text-foreground"
                            x-text="recipeId ? 'Editar Receita' : 'Nova Receita'"></h2>
                        <p class="text-xs text-muted-foreground"
                            x-text="recipeId ? 'Ajuste os detalhes da sua receita' : 'Crie uma nova receita de produção'">
                        </p>
                    </div>
                </div>
                <button @click="closeModal()"
                    class="h-10 w-10 flex items-center justify-center rounded-xl hover:bg-muted transition-colors text-muted-foreground">
            ...
            <div class="p-0 overflow-y-auto flex-1 bg-background/50">
                <form
                    :action="recipeId ? '{{ route('dashboard.producao.receitas.update', '__ID__') }}'.replace('__ID__', recipeId) : '{{ route('dashboard.producao.receitas.store') }}'"
                    method="POST" id="modal-recipe-form" class="space-y-6" @submit.prevent="submitForm">
                    @csrf
                    ...
                    <div class="p-6 space-y-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid gap-4 sm:grid-cols-2">
                                <div class="space-y-2">
                                    ...
                                    <select name="product_id" x-model="formData.product_id" required
                                        class="form-input rounded-xl h-11" @change="onProductChange()">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <select name="variant_id" x-model="formData.variant_id"
                                        class="form-input rounded-xl h-11" @change="onVariantChange()">
                                        <option value="">Produto simples (ou selecione variante)</option>
                                        <template x-for="v in availableVariants" :key="v.id || v.name">
                                            <option :value="String(v.id || v.name)"
                                                x-text="v.name + (v.price ? ' (R$ ' + v.price + ')' : '')"
                                                :data-weight="v.weight_grams"></option>
                                        </template>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="grid gap-4 sm:grid-cols-2">
                                <div class="space-y-2">
                                    ...
                                    <input type="text" name="name" x-model="formData.name" required
                                        class="form-input rounded-xl h-11">
                                </div>
                                <div class="space-y-2">
                                    <label class="text-sm font-medium">Categoria</label>
                                    <input type="text" name="category" x-model="formData.category" list="modal-categories"
                                        class="form-input rounded-xl h-11">
                            ...
                            <div class="grid gap-4 grid-cols-2">
                                <div class="bg-muted/30 p-3 rounded-xl border border-border/50">
                                    <label class="text-[10px] uppercase font-bold text-muted-foreground block mb-1">Peso
                                        Total</label>
                                    <span class="text-lg font-mono font-bold text-primary"
                                        x-text="calculateTotalWeight() + 'g'"></span>
                                    <input type="hidden" name="total_weight" :value="calculateTotalWeight()">
                                </div>
                                <div class="bg-muted/30 p-3 rounded-xl border border-border/50">
                                    <label
                                        class="text-[10px] uppercase font-bold text-muted-foreground block mb-1">Rendimento</label>
                                    <span class="text-lg font-mono font-bold text-foreground"
                                        x-text="calculatePortions() + ' un.'"></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-4 bg-primary/5 rounded-2xl border border-primary/10 space-y-4"
                            x-show="formData.is_bread">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2 text-primary">
                                    <i data-lucide="settings" class="w-4 h-4"></i>
                                    <h3 class="text-sm font-semibold uppercase tracking-wider">Parâmetros de Panificação
                                    </h3>
                                </div>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="uses_baker_percentage"
                                        x-model="formData.uses_baker_percentage" class="rounded-lg text-primary">
                            ...
                            <div class="grid gap-4 sm:grid-cols-2">
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        <input type="number" name="hydration" x-model.number="formData.hydration" step="0.1"
                                            class="form-input rounded-xl pr-10">
                                        <span
                                            class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground text-xs">%</span>
                                    </div>
                                </div>
                                <div class="space-y-2">
                                    <label class="text-xs font-medium">Levain (%)</label>
                                    <div class="relative">
                                        <input type="number" name="levain" x-model.number="formData.levain" step="0.1"
                                            class="form-input rounded-xl pr-10">
                                        <span
                                            class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground text-xs">%</span>
                                    </div>
                                ...
                                <template x-for="(step, sIdx) in formData.steps" :key="sIdx">
                                    <div class="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
                                        <div class="px-4 py-3 bg-muted/20 border-b border-border flex items-center gap-3">
                                            <input type="text" x-model="step.name" :name="'steps['+sIdx+'][name]'"
                                                placeholder="Nome da etapa" required
                                                class="bg-transparent border-none focus:ring-0 font-bold text-sm flex-1 p-0">
                                            <button type="button" @click="removeStep(sIdx)"
                                                x-show="formData.steps.length > 1"
                                                class="text-destructive p-1 hover:bg-destructive/10 rounded-lg transition-colors">
                                        ...
                                        <div class="p-3 sm:p-4 space-y-3">
                                            <template x-for="(ing, iIdx) in step.ingredients" :key="iIdx">
                                                <div
                                                    class="grid grid-cols-2 sm:grid-cols-[1fr,80px,100px,40px] gap-x-3 gap-y-2 items-end p-3 sm:p-0 bg-muted/20 sm:bg-transparent rounded-xl relative group">
                                                    <!-- Ingrediente (Linha Inteira no Mobile) -->
                                                    <div class="col-span-2 sm:col-span-1 space-y-1">
                                                        <label
                                                            class="text-[10px] font-extrabold uppercase text-muted-foreground tracking-wider ml-1">Ingrediente</label>
                                                        <select x-model="ing.ingredient_id"
                                                            :name="'steps['+sIdx+'][ingredients]['+iIdx+'][ingredient_id]'"
                                                            required
                                                            class="form-input rounded-xl text-sm h-10 border-border/50">
                                                    ...
                                                    <div class="col-span-1 sm:col-span-1 space-y-1">
                                                        <label
                                                            class="text-[10px] font-extrabold uppercase text-muted-foreground tracking-wider ml-1 text-center block sm:hidden">%</label>
                                                        <input type="number" step="0.01" x-model="ing.percentage"
                                                            :name="'steps['+sIdx+'][ingredients]['+iIdx+'][percentage]'"
                                                            placeholder="%"
                                                            class="form-input rounded-xl text-sm h-10 text-center border-border/50 font-mono">
                                                    </div>
                                                    <!-- Peso (Meia Linha no Mobile + Delete) -->
                                                    <div class="col-span-1 sm:col-span-1 space-y-1 flex gap-2 items-end">
                                                        <div class="flex-1 space-y-1">
                                                            <label
                                                                class="text-[10px] font-extrabold uppercase text-muted-foreground tracking-wider ml-1 text-center block sm:hidden">Peso
                                                                (g)</label>
                                                            <input type="number" step="0.1" x-model.number="ing.weight"
                                                                :name="'steps['+sIdx+'][ingredients]['+iIdx+'][weight]'"
                                                                placeholder="g"
                                                                class="form-input rounded-xl text-sm h-10 text-center border-border/50 font-mono"
                                                                required>
                                                        </div>
                                                        <button type="button" @click="removeIngredient(sIdx, iIdx)"
                                                            x-show="step.ingredients.length > 1"
                                                            class="h-10 w-10 flex sm:hidden items-center justify-center text-red-500 hover:bg-red-50 rounded-xl transition-colors">
                                                    ...
                                                    <button type="button" @click="removeIngredient(sIdx, iIdx)"
                                                        x-show="step.ingredients.length > 1"
                                                        class="hidden sm:flex h-10 w-10 items-center justify-center text-muted-foreground hover:text-red-500 hover:bg-red-50 rounded-xl transition-all">
                                                        <i data-lucide="x" class="w-4 h-4"></i>
                                                    </button>
                                                </div>
                                            </template>
                                            <button type="button" @click="addIngredient(sIdx)"
                                                class="w-full py-2.5 border border-dashed border-primary/30 rounded-xl text-xs font-bold text-primary hover:bg-primary/5 transition-all flex items-center justify-center gap-2 mt-4">
                        ...
                        <div class="space-y-4 pt-4 border-t border-border">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <label
                                    class="flex items-center gap-3 p-3 rounded-xl border border-border hover:bg-muted/30 transition-colors cursor-pointer">
                                    <input type="checkbox" name="is_active" x-model="formData.is_active"
                                        class="rounded-lg h-5 w-5 text-primary">
                                    <div>
                                ...
                                <label
                                    class="flex items-center gap-3 p-3 rounded-xl border border-border hover:bg-muted/30 transition-colors cursor-pointer">
                                    <input type="checkbox" name="is_bread" x-model="formData.is_bread"
                                        class="rounded-lg h-5 w-5 text-primary">
                                    <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="notes" x-model="formData.notes" rows="2" class="form-input rounded-xl"
                                placeholder="Algum segredo ou dica?"></textarea>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <!-- Footer -->
            <div
                class="p-4 sm:p-6 border-t border-border bg-muted/30 flex flex-col sm:flex-row gap-3 sm:justify-between items-center">
                ...
                <div class="flex gap-3 w-full sm:w-auto">
                    <button type="button" @click="closeModal()"
                        class="flex-1 sm:flex-none btn-outline rounded-xl h-11 px-6">Cancelar</button>
                    <button type="submit" form="modal-recipe-form"
                        class="flex-1 sm:flex-none btn-primary rounded-xl h-11 px-8 shadow-lg shadow-primary/20"
    ...
    <div id="print-queue-modal" x-data="printQueue()" x-show="isOpen" @open-print-queue.window="isOpen = true; loadQueue()"
        @keydown.escape.window="closeModal()"
        class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" x-cloak
        style="display: none;">
        <div
            class="bg-card rounded-xl border border-border shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col m-4">
            <div class="p-6 border-b border-border flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-semibold">Fila de Impressão ({{ date('d') }})</h2>
                    <p class="text-sm text-muted-foreground">Organiza e imprima suas receitas em formato A4. Total: <span
                            x-text="queue.length"></span> unidades</p>
                </div>
            ...
            <div class="p-6 overflow-y-auto flex-1">
                ...
                <div class="space-y-3" x-show="queue.length > 0">
                    <template x-for="(item, index) in queue" :key="index">
                        <div class="flex items-center gap-4 p-4 border border-border rounded-lg">
                            <div
                                class="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-semibold text-sm flex-shrink-0">
                ...
                <div x-show="queue.length === 0" class="text-center py-12 text-muted-foreground">
                    ...
                    <p>Nenhum item na fila. Adicione receitas pela Lista de Produção ou use &quot;Produzir Item&quot; nos
                        cards.</p>
                </div>
            </div>
