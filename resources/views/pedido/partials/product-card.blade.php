...
@if($isPurchasable)
            ...
            <div class="p-0">
                <div class="relative aspect-4-3 overflow-hidden bg-muted cursor-pointer" style="aspect-ratio: 4 / 3;">
                    ...
                    <picture>
                        ...
                        <img src="{{ $imageUrls['jpg'] }}" alt="{{ $product->name }}"
                            loading="{{ $loadEager ? 'eager' : 'lazy' }}" decoding="async" fetchpriority="{{ $fetchPriority }}"
                    ...
                    <div class="flex items-center justify-between mt-auto">
                        ...
                        @if($hasActiveVariants)
                            <button onclick="event.stopPropagation(); openProductModal({{ $product->id }})"
                                class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 w-10 rounded-full shadow-warm">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                                    stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"
                                    class="[&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0">
                        ...
                        @else
                            <button
                                onclick="event.stopPropagation(); addToCart({{ $product->id }}, '{{ addslashes($product->name) }}', {{ $displayPrice }})"
                                class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 w-10 rounded-full shadow-warm">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                                    stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"
                                    class="[&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0">
    ...
    @elseif($displayType === 'list_horizontal')
        <div class="text-card-foreground group overflow-hidden border shadow-sm hover:shadow-xl transition-all duration-300 bg-card rounded-2xl flex flex-col cursor-pointer"
            style="min-width: 140px; max-width: 140px; flex-shrink: 0;" onclick="openProductModal({{ $product->id }})">
            <div class="aspect-square overflow-hidden bg-muted relative">
                <div class="absolute inset-0 bg-gradient-to-br from-gray-200 via-gray-100 to-gray-200"
                    id="placeholder-h-{{ $product->id }}">
                    <div class="absolute inset-0 flex items-center justify-center">
                ...
                <picture>
                    <source srcset="{{ $imageUrls['webp'] }}" type="image/webp">
                    <img src="{{ $imageUrls['jpg'] }}" alt="{{ $product->name }}" loading="{{ $loadEager ? 'eager' : 'lazy' }}"
                        decoding="async" fetchpriority="{{ $fetchPriority }}" width="{{ $imgWidth }}" height="{{ $imgHeight }}"
            ...
            <div class="p-2.5 flex flex-col gap-1.5">
                ...
                <div class="flex items-center justify-between mt-auto pt-1">
                    ...
                    @if($hasActiveVariants)
                        <button onclick="event.stopPropagation(); openProductModal({{ $product->id }})"
                            class="inline-flex items-center justify-center gap-1 whitespace-nowrap text-xs font-medium bg-primary text-white hover:bg-primary/90 rounded-lg h-7 w-7 shadow-sm hover:shadow transition-all duration-200 flex-shrink-0">
    ...
    @elseif($displayType === 'list_vertical')
        <div class="text-card-foreground group overflow-hidden border shadow-sm hover:shadow-xl transition-all duration-300 bg-card rounded-2xl flex flex-row gap-4 cursor-pointer"
            onclick="openProductModal({{ $product->id }})">
            <div class="w-24 h-24 flex-shrink-0 overflow-hidden bg-muted relative rounded-lg">
                <div class="absolute inset-0 bg-gradient-to-br from-gray-200 via-gray-100 to-gray-200"
                    id="placeholder-v-{{ $product->id }}">
                    <div class="absolute inset-0 flex items-center justify-center">
            ...
            <div class="flex-1 flex flex-col justify-between py-3">
                ...
                <div class="flex items-center justify-between mt-auto">
                    ...
                    @if($hasActiveVariants)
                        <button onclick="event.stopPropagation(); openProductModal({{ $product->id }})"
                            class="inline-flex items-center justify-center gap-1 whitespace-nowrap text-xs font-medium bg-primary text-white hover:bg-primary/90 rounded-lg h-9 w-9 shadow-sm hover:shadow transition-all duration-200 flex-shrink-0">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M5 12h14"></path>
                                <path d="M12 5v14"></path>
