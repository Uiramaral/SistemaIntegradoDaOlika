...
@section('title', 'Editar Cliente - OLIKA Painel')
...
@endif
    ...
    <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
        ...
        <div class="p-6 pt-0">
            <form action="{{ route('dashboard.customers.update', is_array($customer) ? $customer['id'] : $customer->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="space-y-2">
                        ...
                        <input 
                            ...
                            id="name" 
                            name="name" 
                            value="{{ old('name', is_array($customer) ? ($customer['name'] ?? '') : ($customer->name ?? '')) }}" 
                    ...
                    <div class="space-y-2">
                        ...
                        <input 
                            ...
                            id="email" 
                            name="email" 
                            value="{{ old('email', is_array($customer) ? ($customer['email'] ?? '') : ($customer->email ?? '')) }}" 
                            class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                        @error('email')
                            <p class="text-sm text-destructive">{{ $message }}</p>
                    ...
                    <div class="space-y-2">
                        ...
                        <input 
                            ...
                            id="phone" 
                            name="phone" 
                            value="{{ old('phone', is_array($customer) ? ($customer['phone'] ?? '') : ($customer->phone ?? '')) }}" 
                            class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                            required
                        >
                        @error('phone')
                    ...
                    <div class="space-y-2">
                        ...
                        <input 
                            ...
                            id="cpf" 
                            name="cpf" 
                            value="{{ old('cpf', is_array($customer) ? ($customer['cpf'] ?? '') : ($customer->cpf ?? '')) }}" 
                            class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                        @error('cpf')
                            <p class="text-sm text-destructive">{{ $message }}</p>
                    ...
                    <div class="space-y-2">
                        ...
                        <input 
                            ...
                            id="birth_date" 
                            name="birth_date" 
                            value="{{ old('birth_date', is_array($customer) ? ($customer['birth_date'] ?? '') : ($customer->birth_date ?? '')) }}" 
                            class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                        @error('birth_date')
                            <p class="text-sm text-destructive">{{ $message }}</p>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        @php
                            $customerId = is_array($customer) ? ($customer['id'] ?? 0) : ($customer->id ?? 0);
                            $currentBalance = \App\Models\CustomerCashback::getBalance($customerId);
                        @endphp
                        <div class="space-y-2">
                            <div class="flex items-center gap-2 text-sm text-muted-foreground">
                                <span>Saldo atual: <strong class="text-foreground">R$ {{ number_format($currentBalance, 2, ',', '.') }}</strong></span>
                            </div>
                            <input 
                                type="number" 
                                id="cashback_balance" 
                                ...
                                step="0.01"
                                min="0"
                                value="{{ old('cashback_balance', number_format($currentBalance, 2, '.', '')) }}" 
                                class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                                placeholder="0.00"
                            >
                            <p class="text-xs text-muted-foreground">Defina o valor de cashback que o cliente deve ter. O sistema ajustará automaticamente criando uma transação de ajuste se necessário.</p>
                        </div>
                        @error('cashback_balance')
                            <p class="text-sm text-destructive">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
