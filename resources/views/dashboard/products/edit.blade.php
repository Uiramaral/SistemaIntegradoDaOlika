...
@section('title', 'Editar Produto - OLIKA Dashboard')
...
<div class="space-y-6 animate-in fade-in duration-500">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('dashboard.products.show', $product) }}" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2">
                Ver Produto
            </a>
    ...
    <form action="{{ route('dashboard.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6 pb-24">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                        @error('name')<span class="text-sm text-red-600">{{ $message }}</span>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2">SKU (Código)</label>
                        <input type="text" name="sku" value="{{ old('sku', $product->sku) }}" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                    ...
                    <div>
                        ...
                        <input type="number" name="price" step="0.01" min="0" value="{{ old('price', $product->price) }}" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                        @error('price')<span class="text-sm text-red-600">{{ $message }}</span>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2">Peso (g)</label>
                        <input type="number" name="weight_grams" step="1" min="0" value="{{ old('weight_grams', $product->weight_grams) }}" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                    ...
                    <div>
                        ...
                        <input type="number" name="stock" min="0" value="{{ old('stock', $product->stock) }}" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                    ...
                    <div>
                        ...
                        <input type="number" name="preparation_time" min="0" value="{{ old('preparation_time', $product->preparation_time) }}" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                ...
                <div>
                    ...
                    <textarea name="ingredients" rows="3" placeholder="Ex.: Farinha de trigo, água, fermento natural, sal..." class="flex min-h-[60px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">{{ old('ingredients', $product->nutritional_info['ingredients'] ?? '') }}</textarea>
        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
            <div class="p-6 space-y-6">
                ...
                <p class="text-sm text-muted-foreground">Edite opções de peso/sabor com preços.</p>
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="gluten_free" value="1" @checked(old('gluten_free', $product->gluten_free)) class="rounded border-gray-300">
                            <span class="text-sm">Produto sem glúten</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="contamination_risk" value="1" @checked(old('contamination_risk', $product->contamination_risk)) class="rounded border-gray-300">
                                    ...
                                    <div class="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
                                        @foreach($groupAllergens as $allergen)
                                            <label class="flex items-center gap-2 p-2 rounded border hover:bg-accent cursor-pointer">
                                                <input type="checkbox" name="allergen_ids[]" value="{{ $allergen->id }}" @checked(in_array($allergen->id, old('allergen_ids', $selectedAllergens ?? []))) class="rounded border-gray-300">
        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
            <div class="p-6 space-y-6">
                ...
                @if($product->cover_image || ($product->images && $product->images->count() > 0))
                    <div class="mb-4">
                        <h3 class="text-sm font-medium mb-2">Imagens Atuais</h3>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @if($product->cover_image)
                                <div class="relative group">
                                    <img src="{{ asset('storage/' . $product->cover_image) }}" alt="Capa" class="w-full h-32 object-cover rounded-lg border">
                                    <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center">
                                        <span class="text-white text-xs">Imagem de Capa</span>
                                    </div>
                                </div>
                            @endif
                            @foreach($product->images as $image)
                                <div class="relative group" data-image-id="{{ $image->id }}">
                                    <img src="{{ asset('storage/' . $image->path) }}" alt="Imagem" class="w-full h-32 object-cover rounded-lg border">
                                    <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-2">
                                        @if(!$image->is_primary)
                                            <button type="button" onclick="setPrimary({{ $image->id }})" class="text-white text-xs bg-blue-600 px-2 py-1 rounded">Principal</button>
                                        @endif
                                        <button type="button" onclick="deleteImage({{ $image->id }})" class="text-white text-xs bg-red-600 px-2 py-1 rounded">Excluir</button>
                                    </div>
                                    @if($image->is_primary)
                ...
                <div>
                    <label class="block text-sm font-medium mb-2">Nova Imagem de Capa (substitui a atual)</label>
                ...
                <div>
                    <label class="block text-sm font-medium mb-2">Adicionar Mais Imagens</label>
        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
            <div class="p-6 space-y-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $product->is_featured)) class="rounded border-gray-300">
...
@php($__variantsForJs = $product->variants()->get(['id','name','price','sku','is_active','sort_order'])->toArray())
<script>
let currentCropType = null; // 'cover' ou 'image'
let currentImageIndex = null;
let cropperInstance = null;
let croppedImages = {}; // Armazena as imagens cortadas
let pendingImages = []; // Fila de imagens pendentes para processar
...
(function() {
    ...
    if (hasCroppedCover) {
        ...
        console.log('ℹ️ Nenhuma nova imagem de capa para enviar - mantendo a atual');
    }
    ...
    // Adicionar _method PUT para Laravel (já que vamos usar POST)
    ...
    // Enviar via fetch usando POST (Laravel processa PUT via _method)
    fetch(form.action, {
        method: 'POST', // Sempre usar POST para uploads
        body: formData,
        headers: {
...
async function deleteImage(imageId) {
    ...
    try {
        const response = await fetch(`{{ route('dashboard.products.images.delete', ['product' => $product, 'image' => 'IMAGE_ID']) }}`.replace('IMAGE_ID', imageId), {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
...
async function setPrimary(imageId) {
    try {
        const response = await fetch(`{{ route('dashboard.products.images.set-primary', ['product' => $product, 'image' => 'IMAGE_ID']) }}`.replace('IMAGE_ID', imageId), {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'Accept': 'application/json'
...
function addVariantRow(v={}){
  ...
  row.innerHTML = `
    ...
    <label class="flex items-center gap-2"><input type="checkbox" name="variant_active[${idx}]" value="1" ${(v.is_active === true || v.is_active === 1 || (v.is_active !== false && v.is_active !== 0 && v.is_active !== '0')) ? 'checked' : ''}> Ativo</label>
    ${v.id ? `<button type="button" onclick="deleteVariant(${v.id}, ${idx})" class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 text-sm">Excluir</button>` : `<button type="button" onclick="removeVariantRow(${idx})" class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 text-sm">Remover</button>`}
  `;
...
async function deleteVariant(variantId, idx) {
  if (!confirm('Tem certeza que deseja excluir esta variante permanentemente?')) {
    return;
  }
  
  try {
    const productId = {{ $product->id }};
    const url = `/dashboard/products/${productId}/variants/${variantId}`;
    ...
    if (response.ok) {
      ...
      const data = await response.json();
      alert('Erro ao excluir variante: ' + (data.message || 'Erro desconhecido'));
    }
  } catch (error) {
    console.error('Erro:', error);
    alert('Erro ao excluir variante: ' + error.message);
  }
}
