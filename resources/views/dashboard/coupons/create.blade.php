...
@section('title', 'Novo Cupom - OLIKA Dashboard')

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold tracking-tight">Novo Cupom</h1>
            <p class="text-muted-foreground">Crie um novo cupom de desconto</p>
        </div>
    ...
    <form action="{{ route('dashboard.coupons.store') }}" method="POST" class="space-y-6">
        ...
        <div class="rounded-lg border bg-card p-6 space-y-6">
            ...
            <div>
                ...
                <div class="mt-4">
                    ...
                    <textarea name="description" rows="2" class="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">{{ old('description') }}</textarea>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="value" step="0.01" min="0" value="{{ old('value') }}" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                    </div>
                </div>
                <div class="mt-4">
                    <label class="block text-sm font-medium mb-2">Valor Mínimo do Pedido (opcional)</label>
                    <input type="number" name="minimum_amount" step="0.01" min="0" value="{{ old('minimum_amount') }}" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="visibility" id="visibility" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                            <option value="public" {{ old('visibility') == 'public' ? 'selected' : '' }}>Público (visível para todos elegíveis)</option>
                            <option value="private" {{ old('visibility') == 'private' ? 'selected' : '' }}>Privado (precisa digitar código)</option>
                    ...
                    <div id="targetCustomerField" class="{{ old('visibility') == 'targeted' ? '' : 'hidden' }}">
                        ...
                        <select name="target_customer_id" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                            ...
                            <option value="{{ $customer->id }}" {{ old('target_customer_id') == $customer->id ? 'selected' : '' }}>{{ $customer->name }} ({{ $customer->email }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="mt-4 space-y-2">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="first_order_only" value="1" {{ old('first_order_only') ? 'checked' : '' }} class="h-4 w-4">
                        <span class="text-sm font-medium">Apenas para primeiro pedido</span>
                    </label>
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="free_shipping_only" value="1" {{ old('free_shipping_only') ? 'checked' : '' }} class="h-4 w-4">
            ...
            <div>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="datetime-local" name="starts_at" value="{{ old('starts_at') }}" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2">Data de Expiração (opcional)</label>
                        <input type="datetime-local" name="expires_at" value="{{ old('expires_at') }}" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                    </div>
                </div>
            </div>
        ...
        <div class="flex justify-end gap-4">
            ...
            <button type="submit" class="px-4 py-2 rounded-md bg-primary text-primary-foreground hover:bg-primary/90">Criar Cupom</button>
        </div>
    </form>
</div>
