...
<div class="space-y-6">
    <div>
        <h1 class="text-3xl font-bold tracking-tight">Nova Categoria</h1>
        <p class="text-muted-foreground">Adicione uma nova categoria para organizar seus produtos</p>
    </div>
    ...
    <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
        <form action="{{ route('dashboard.categories.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            
            <div>
                <label class="block text-sm font-medium mb-2">Nome da Categoria *</label>
                <input type="text" name="name" value="{{ old('name') }}" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                @error('name')<span class="text-sm text-red-600 mt-1 block">{{ $message }}</span>@enderror
            </div>

            <div>
                <label class="block text-sm font-medium mb-2">Descrição</label>
                <textarea name="description" rows="4" class="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">{{ old('description') }}</textarea>
            ...
            <div>
                ...
                <input type="url" name="image_url" value="{{ old('image_url') }}" placeholder="https://exemplo.com/imagem.jpg" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                ...
                <div>
                    ...
                    <select name="display_type" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                        <option value="grid" {{ old('display_type', 'grid') === 'grid' ? 'selected' : '' }}>Grid (Grade)</option>
                        <option value="list_horizontal" {{ old('display_type', 'grid') === 'list_horizontal' ? 'selected' : '' }}>Lista Horizontal (Rolagem)</option>
                        <option value="list_vertical" {{ old('display_type', 'grid') === 'list_vertical' ? 'selected' : '' }}>Lista Vertical</option>
                    </select>
