...
<div class="space-y-6 w-full min-w-0 max-w-full overflow-hidden">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            <a href="{{ route('dashboard.orders.index') }}" class="inline-flex items-center justify-center rounded-md p-2 hover:bg-gray-100 transition-colors text-gray-600 hover:text-gray-900">
                <i data-lucide="arrow-left" class="w-5 h-5"></i>
            </a>
            <h1 class="text-2xl font-semibold">Pedido #{{ $order->order_number }}</h1>
        </div>
        <div class="flex items-center gap-2">
            <button type="button" id="btn-open-receipt" data-order-id="{{ $order->id }}" class="inline-flex items-center justify-center h-9 w-9 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 transition-colors" title="Ver Recibo">
            ...
            @if($order->payment_status !== 'refunded' && optional($order->customer)->phone)
                <form method="POST" action="{{ route('dashboard.orders.sendReceipt', $order) }}" class="inline">
                    @csrf
                    <button type="submit" class="inline-flex items-center justify-center h-9 w-9 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 transition-colors" title="Enviar recibo via WhatsApp">
                        <i data-lucide="send" class="w-4 h-4"></i>
                    </button>
                </form>
            @endif
            <div class="relative inline-flex items-center gap-2 pl-3 pr-4 h-9 rounded-lg border border-gray-300 bg-white">
                <div class="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                    <i data-lucide="user" class="w-4 h-4 text-primary"></i>
                </div>
                <div class="flex flex-col items-start min-w-0">
                    <span class="text-xs font-medium text-gray-900 truncate max-w-[120px]">{{ auth()->user()->name ?? 'Admin' }}</span>
                    <span class="text-[10px] text-gray-500 truncate max-w-[120px]">{{ auth()->user()->email ?? 'admin@olika.com.br' }}</span>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="p-4 sm:p-6 space-y-3">
                    <div class="space-y-3" id="items-tbody">
                        @forelse($order->items as $item)
                            <div class="flex items-center justify-between py-2 border-b border-border/50 last:border-0" data-item-id="{{ $item->id }}">
                                <div class="flex items-center gap-3 flex-1 min-w-0">
                                    ...
                                    <span class="text-sm font-semibold item-total-price whitespace-nowrap flex-shrink-0">R$ {{ number_format($item->total_price, 2, ',', '.') }}</span>
                                    <button type="button" class="btn-remove-item inline-flex items-center justify-center rounded-md p-1 hover:bg-destructive/10 hover:text-destructive text-muted-foreground flex-shrink-0" data-order-id="{{ $order->id }}" data-item-id="{{ $item->id }}" title="Remover item">
                ...
                <div class="p-4 sm:p-6">
                    <form action="{{ route('dashboard.orders.updateStatus', $order) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        ...
                        <div>
                            ...
                            <textarea name="note" rows="3" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" placeholder="Adicione uma observação sobre esta mudança..."></textarea>
                        </div>
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="skip_notification" value="1" id="skip_notification_status" class="h-4 w-4 text-primary rounded border-input">
                            <label for="skip_notification_status" class="text-sm font-medium cursor-pointer">Atualizar sem enviar notificação</label>
                        </div>
        ...
        <div class="space-y-6 min-w-0 lg:col-span-1">
            ...
            <div class="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                ...
                <div class="p-4 sm:p-6 space-y-4">
                            ...
                            <div class="flex items-center justify-between p-3 border border-border rounded-lg">
                                <div class="flex-1">
                                    <p class="text-sm font-medium">
                                        {{ $coupon->name }} {{ number_format($coupon->value, 2, ',', '.') }}{{ $coupon->type === 'percentage' ? '%' : '' }}
                                    </p>
                                </div>
                                <form action="{{ route('dashboard.orders.applyCoupon', $order) }}" method="POST" class="inline">
                                    @csrf
                                    <input type="hidden" name="coupon_code" value="{{ $coupon->code }}">
                                    <button type="submit" class="text-sm text-muted-foreground hover:text-primary transition-colors">Aplicar</button>
                                </form>
                            </div>
                            @endforeach
                        </div>
                    ...
                    @endif
                    <div>
                        <form action="{{ route('dashboard.orders.applyCoupon', $order) }}" method="POST" class="flex gap-2">
                            @csrf
                            <input type="text" name="coupon_code" placeholder="Digite o código do cupom" class="flex-1 rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
            ...
            <div class="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                ...
                <div class="p-4 sm:p-6">
                    <form action="{{ route('dashboard.orders.applyDiscount', $order) }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="text-sm font-medium mb-2 block">Tipo de Desconto</label>
                            <select name="discount_type" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        ...
                        <div>
                            ...
                            <input type="number" name="discount_value" step="0.01" min="0" placeholder="Ex: 10 ou 10.50" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        </div>
                        <button type="submit" class="w-full btn-primary h-10">
                            Aplicar Desconto
                        </button>
            ...
            <div class="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                ...
                <div class="p-4 sm:p-6 space-y-2">
                    ...
                    <div class="text-sm">
                        ...
                        @endphp
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ml-2 {{ $paymentStatusClass }}">
            ...
            <div class="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                ...
                <div class="p-4 sm:p-6">
                    <form action="{{ route('dashboard.orders.adjustDeliveryFee', $order) }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                        ...
                        <div>
                            ...
                            <input type="number" name="delivery_fee" step="0.01" min="0" value="{{ $order->delivery_fee ?? 0 }}" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        </div>
                        <div>
                            <label class="text-sm font-medium mb-2 block">Motivo do Ajuste (opcional)</label>
                            <textarea name="reason" rows="2" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" placeholder="Ex: Desconto especial, erro no cálculo..."></textarea>
                        </div>
                        <button type="submit" class="w-full btn-primary h-10">
                            Atualizar Taxa
                        </button>
                ...
                <div class="p-4 sm:p-6">
                    <form action="{{ route('dashboard.orders.update', $order) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="3" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" placeholder="Observações visíveis apenas para a equipe...">{{ $order->notes ?? '' }}</textarea>
                        </div>
                        
                        <div>
                            <label class="text-sm font-medium mb-2 block">Observações do Cliente</label>
                            <textarea name="observations" rows="3" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" placeholder="Observações do cliente...">{{ $order->observations ?? '' }}</textarea>
                        </div>
                        
                        <div>
                            <label class="text-sm font-medium mb-2 block">Instruções de Entrega</label>
                            <textarea name="delivery_instructions" rows="3" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" placeholder="Instruções especiais para entrega...">{{ $order->delivery_instructions ?? '' }}</textarea>
                        </div>
                        
                        <div class="space-y-2">
                            <label class="text-sm font-medium">Data e Hora de Entrega Agendada</label>
                            <div class="flex gap-2 items-center">
                                <input type="date" id="scheduled_delivery_date" value="{{ $order->scheduled_delivery_at ? $order->scheduled_delivery_at->format('Y-m-d') : '' }}" class="flex-1 rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                                <input type="time" id="scheduled_delivery_time" value="{{ $order->scheduled_delivery_at ? $order->scheduled_delivery_at->format('H:i') : '' }}" class="flex-1 rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                            </div>
                        ...
                        <div class="space-y-3 pt-2">
                            <div class="flex items-center gap-2">
                                <input type="checkbox" id="create_payment" name="create_payment" value="1" class="h-4 w-4 text-primary rounded border-input">
                                <label for="create_payment" class="text-sm font-medium cursor-pointer">Criar nova cobrança para este pedido</label>
                            </div>
                            
                            <div id="payment_methods" class="hidden space-y-3 pl-6">
                                <div>
                                    <label class="text-sm font-medium mb-2 block">Método de Pagamento</label>
                                    <select name="payment_method" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                            ...
                            <div class="flex items-center gap-2">
                                <input type="checkbox" id="send_whatsapp" name="send_whatsapp" value="1" checked class="h-4 w-4 text-primary rounded border-input">
                                <label for="send_whatsapp" class="text-sm font-medium cursor-pointer">Enviar notificação via WhatsApp ao cliente</label>
                            </div>
                            
                            <div id="whatsapp_message" class="space-y-2">
                                <label class="text-sm font-medium">Mensagem personalizada (opcional)</label>
                                <textarea name="whatsapp_message" rows="3" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" placeholder="Deixe em branco para usar mensagem padrão">Olá! Seu pedido {{ $order->order_number }} foi atualizado. Valor total: R$ {{ number_format($order->final_amount ?? $order->total_amount, 2, ',', '.') }}</textarea>
                                <p class="text-xs text-muted-foreground">O link de pagamento será adicionado automaticamente se uma cobrança for criada.</p>
                            </div>
                            
                            <div class="flex items-center gap-2">
                                <input type="checkbox" id="skip_notification" name="skip_notification" value="1" class="h-4 w-4 text-primary rounded border-input">
                                <label for="skip_notification" class="text-sm font-medium cursor-pointer">Salvar sem enviar notificação ao cliente</label>
                            </div>
...
<div id="add-item-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            ...
            <form action="{{ route('dashboard.orders.addItem', $order) }}" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select id="product-select" name="product_id" required class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    ...
                    <div id="normal-item-fields">
                        <div>
                            ...
                            <input type="number" name="quantity" id="quantity" min="1" value="1" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        </div>

                        <div>
                            <label class="block text-sm font-medium mb-2">Preço Unitário (opcional)</label>
                            <input type="number" name="unit_price" id="unit_price" step="0.01" min="0" placeholder="Deixe em branco para usar o preço padrão" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        ...
                        <div>
                            ...
                            <input type="text" name="custom_name" maxlength="255" placeholder="Ex: Focaccia Especial" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        </div>

                        <div>
                            <label class="block text-sm font-medium mb-2">Observações Especiais (opcional)</label>
                            <textarea name="special_instructions" id="special_instructions" rows="2" maxlength="500" placeholder="Ex: Com pouco sal, sem azeitona" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"></textarea>
                        </div>
                    </div>
                </div>
