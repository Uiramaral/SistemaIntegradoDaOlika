...
@section('quick-filters')
  <form method="GET" action="{{ route('admin.dashboard') }}" style="display:flex;gap:8px">
    ...
    <select name="status" class="pill" style="padding:6px 12px">
      ...
      <option value="waiting_payment" @selected(request('status')==='waiting_payment')>Aguardando Pagamento</option>
      ...
      <option value="delivering" @selected(request('status')==='delivering')>Entregando</option>
  ...
  <div class="card" style="padding:0">
    <div style="padding:16px;border-bottom:1px solid var(--color-border);display:flex;align-items:center;justify-content:space-between">
      ...
      <form method="GET" style="display:flex;align-items:center;gap:8px">
        <input type="search" name="q" value="{{ request('q') }}" placeholder="Buscar cliente…" style="padding:6px 12px;border-radius:8px;border:1px solid var(--color-border);font-size:14px" />
