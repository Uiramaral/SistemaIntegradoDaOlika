...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600&family=Outfit:wght@600;700&display=swap"
    rel="stylesheet">
...
<body class="bg-[var(--background)]">
  ...
  <div class="flex min-h-screen">
    <aside id="sidebar"
      class="sidebar fixed md:static inset-y-0 left-0 transition-transform duration-300 z-40 -translate-x-full md:translate-x-0">
          ...
          <ul class="space-y-1">
            <li><a href="{{ route('dashboard.index') }}"
                class="{{ request()->routeIs('dashboard.index') ? 'active' : '' }}"><i
                  data-lucide="layout-dashboard"></i> Visão Geral</a></li>
            <li><a href="{{ route('dashboard.orders.index') }}"
                class="{{ request()->routeIs('dashboard.orders.*') ? 'active' : '' }}"><i data-lucide="receipt"></i>
                Pedidos</a></li>
            <li><a href="{{ route('dashboard.customers.index') }}"
                class="{{ request()->routeIs('dashboard.customers.*') ? 'active' : '' }}"><i data-lucide="users"></i>
                Clientes</a></li>
            <li><a href="{{ route('dashboard.deliveries.index') }}"
                class="{{ request()->routeIs('dashboard.deliveries.*') ? 'active' : '' }}"><i data-lucide="truck"></i>
                Entregas</a></li>
          </ul>
        </div>
      </nav>
      ...
      <div class="px-4 py-4 border-t border-[var(--sidebar-border)]">
        <form method="POST" action="{{ route('auth.logout') }}">
          @csrf
          <button type="submit"
            class="flex items-center gap-3 text-[var(--sidebar-foreground)] hover:text-white w-full">
