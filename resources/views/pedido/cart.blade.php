                ...
                <div class="lg:sticky lg:top-24 lg:h-fit">
                    <div class="rounded-lg border bg-card text-card-foreground shadow-sm shadow-warm-lg">
                        ...
                        <div class="p-6 pt-0 space-y-4">
                            ...
                            <!-- Botão Finalizar -->
                            <a id="checkoutBtn" href="{{ route('pedido.checkout.index') }}"
                            ...
                            <p class="text-xs text-center text-muted-foreground">Entrega grátis para pedidos acima de R$ 50
                            </p>
                        </div>
                    </div>
                </div>
        ...
        async function loadCart() {
            try {
                ...
                if (minFree > 0 || cashbackPercent > 0) {
                    if (benefits) benefits.classList.remove('hidden');
                    // Frete grátis
                    if (minFree > 0 && fsWrap) {
                        fsWrap.classList.remove('hidden');
                        if (fsProg) fsProg.style.width = `${progress}%`;
                        if (fsHint) fsHint.textContent = remaining > 0 ? `Faltam R$ ${formatBRL(remaining)}` : 'Frete grátis alcançado!';
                    } else if (fsWrap) {
                        fsWrap.classList.add('hidden');
                    }
                    // Cashback
                    if (cashbackPercent > 0 && cbWrap) {
                        cbWrap.classList.remove('hidden');
                        if (cbAmt) cbAmt.textContent = 'R$ ' + formatBRL(cashbackAmount);
                        if (cbPct) cbPct.textContent = cashbackPercent.toFixed(0);
                    } else if (cbWrap) {
